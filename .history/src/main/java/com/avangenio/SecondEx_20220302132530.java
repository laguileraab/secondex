...
public class SecondEx {
            ...
            switch (key.toUpperCase()) {
                case "CITY":
                    if (FileUtils.DataisEmpty()) {
                        ...
                        if (datas.isEmpty()) {
                            try {
                                fetchData(fileUtils,"data.tmp");


                                datas.addAll(fileUtils.getDatas());
    ...
    private static FileUtils fetchData(FileUtils fileUtils, String string) throws FileNotFoundException {
        fileUtils.setRoute("data.tmp");
        String tmp = fileUtils.ReadFile();
        tmp = tmp.substring(0, tmp.length() - 1);
        fileUtils.setRoute("data.tmp");
        fileUtils.setText(fileUtils.ReadFile());
        fileUtils.ReadBlock("F1");
        fileUtils.ReadBlock("F2");
        return fileUtils;
    }
}
