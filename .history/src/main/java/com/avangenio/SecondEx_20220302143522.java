...
public class SecondEx {
                        ...
                        try {
                            ...
                            for (int j = 0; j < fileUtils.getDatas().size(); j++) {
                                if (fileUtils.getDatas().get(j).getCity().toUpperCase()
                                    ...
                                    if (dataShow.isEmpty() | dataShow.contains(fileUtils.getDatas().get(j).getCity())) {
                                        System.out.println(fileUtils.getDatas().get(j).getName() + ","
                                                + fileUtils.getDatas().get(j).getId());
                                        dataShow.add(fileUtils.getDatas().get(j).getCity());
                                    }
                                }
                            }
    ...
    private static FileUtils fetchData(FileUtils fileUtils, String route) throws FileNotFoundException {
        ...
        datas = FileUtils.FormatID(datas, "[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]-[A-Z]"); // Format id for f2 //
                                                                                             // format
        datasAux.addAll(FileUtils.ReadFields(f1, "F1", datas, ",")); // Add all objects found
