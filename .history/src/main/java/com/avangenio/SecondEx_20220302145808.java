...
public class SecondEx {
    ...
    private static FileUtils fetchData(FileUtils fileUtils, String route) throws FileNotFoundException {
        ...
        datas.addAll(FileUtils.ReadFields(f1, "F1",new ArrayList<Data>() , ",")); // Add all objects found
        datasAux = FileUtils.FormatID(new ArrayList<Data>(), "[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]-[A-Z]"); // Format id for f2
        datas.addAll(FileUtils.ReadFields(f2, "F2", datasAux, " ; ")); // Store in object data f2
